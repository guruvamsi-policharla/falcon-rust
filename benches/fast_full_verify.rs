...
fn benchmark_stream(c: &mut Criterion) {
    ...
    let index_counts = [1, 2, 4, 8];
    let invalid_fractions = [0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.99];
                    ...
                    |b| {
                        b.iter(|| {
                            ...
                            for (i, (_, msg, _)) in all_data.iter().enumerate() {
                                if falcon512::fverify_fullverify(
                                    msg,
                                    &expanded_sigs[i],
                                    &pk,
                                    &indices,
                        ...
                        |b| {
                            b.iter(|| {
                                ...
                                for (_, msg, sig) in &all_data {
                                    if falcon512::verify(msg, sig, &pk) {
                                        count += 1;
                                    }
                                }
                                count
                ...
                group.bench_function(
                    ...
                    |b| {
                        b.iter(|| {
                            let mut count = 0;
                            for (i, (_, msg, _)) in all_data.iter().enumerate() {
                                if falcon1024::fverify_fullverify(
                                    msg,
                                    &expanded_sigs[i],
                                    &pk,
                                    &indices,
                    ...
                    group.bench_function(
                        ...
                        |b| {
                            b.iter(|| {
                                let mut count = 0;
                                for (_, msg, sig) in &all_data {
                                    if falcon1024::verify(msg, sig, &pk) {
                                        count += 1;
                                    }
                                }
                                count
