...
def main():
    ...
    # Load C FFI verify 512
    ffi_verify_path = criterion_dir / "c ffi" / "verify 512"
    ffi_verify = load_criterion_estimate(ffi_verify_path)
    print(f"C FFI verify 512: {ffi_verify['mean']/1000:.2f} µs")
    ...
    verify_mean = verify_512["mean"] / 1000
    verify_ci = (verify_512["ci_upper"] - verify_512["ci_lower"]) / 2 / 1000
    ...
    ax.errorbar(indices, means, yerr=[errors_lower, errors_upper], 
                ...
                capsize=4, capthick=1.5, label='fverify 512 (fast verify)',
                zorder=3)
    
    # Plot verify 512 baseline as horizontal line with uncertainty band
    ax.axhline(y=verify_mean, color='#E94F37', linestyle='--', linewidth=2,
               label=f'verify 512 (Rust): {verify_mean:.1f} µs', zorder=2)
    ...
    ax.axhline(y=ffi_mean, color='#8B5CF6', linestyle=':', linewidth=2,
               label=f'verify 512 (C FFI): {ffi_mean:.1f} µs', zorder=2)
    ax.fill_between(ax.get_xlim(), ffi_mean - ffi_ci, ffi_mean + ffi_ci,
                    color='#8B5CF6', alpha=0.15, zorder=1)
    
    # Find and annotate breakeven point (linear interpolation)
    # Where fverify crosses verify_512
    ...
    ax.set_title('Falcon-512: Fast Verify Performance vs Number of Indices\n'
    ...
    # Set y-axis to start from 0 for honest comparison
    ...
    # Save the figure
    output_path = Path("/Users/vamsi/Github/falcon-rust/benchmark/fverify_512_benchmark.png")
